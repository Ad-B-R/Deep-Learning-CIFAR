...
for j in range(X_test_flatten.shape[0]):
    ...
    topk_index = torch.topk(dists,k,largest=False).indices
    index = torch.mode(Y_train[topk_index]).values
    
    correct+=(Y_test[j]==index).float()
print(correct*100/X_test_flatten.shape[0])

meta = cifar.unpickle(os.path.join('cifar-10-batches-py','batches.meta'))
print(meta[b'label_names'][cifar.Y_test[1]])
img = cifar.X_test[1].permute(1,2,0)
img = img.numpy()/255.0
plt.imshow(img)
plt.axis("off")
plt.show()
